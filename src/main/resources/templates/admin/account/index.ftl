...
<body class="gray-bg">
		                            ...
		                            <div class="example">
		                                <div class="bootstrap-table"><div class="fixed-table-toolbar"><div class="bars pull-left"><div class="btn-group hidden-xs" id="exampleToolbar" role="group">
		                                	<@shiro.hasPermission name="content:secret:add">
				                        		<button class="btn btn-outline btn-default" type="button" onclick="add();"><i class="glyphicon glyphicon-ok" aria-hidden="true"></i>&nbsp;标记为已读</button>
				                        	</@shiro.hasPermission>
		                                </div>
		                              </div>
		                              ...
		                              <div class="pull-right search"><input id="searchText" class="form-control input-outline" type="text" placeholder="输入手机号"></div>
		                              </div>
    ...
    <script>
        ...
        function initTable() {
			...
			$("#table_list").bootstrapTable({
			    ...
			    columns: [{
			        ...
			        field: "empty",
                    formatter: function (value, row, index) {
                    	var operateHtml = '<@shiro.hasPermission name="account:index"><button class="btn btn-primary btn-xs" type="button" onclick="detail(\''+row.id+'\')"><i class="fa fa-search"></i>&nbsp;查看</button> &nbsp;</@shiro.hasPermission>';
                        return operateHtml;
                    }
			    }]
			});
        ...
        function detail(id){
        	location.href = '${ctx!}/admin/account/detail/' + id;
        }
        function add(){
        	location.href = '${ctx!}/admin/account/add';
        }
        function updateSort(id, upOrDown) {
        	...
        	$.ajax({
	    		   ...
	    		   url: "${ctx!}/admin/account/updateOrder",
