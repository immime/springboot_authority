...
<body class="gray-bg">
    ...
    <script>
        ...
        function initTable() {
			...
			$("#table_list").bootstrapTable({
			    ...
			    columns: [{
			        ...
			        field: "empty",
                    formatter: function (value, row, index) {
                    	var operateHtml = '<@shiro.hasPermission name="feedback:index"><button class="btn btn-primary btn-xs" type="button" onclick="detail(\''+row.id+'\')"><i class="fa fa-search"></i>&nbsp;查看</button> &nbsp;</@shiro.hasPermission>';
                        return operateHtml;
                    }
			    }]
			});
        ...
        function detail(id){
        	location.href = '${ctx!}/admin/feedback/detail/' + id;
        }
        function add(){
        ...
        function updateSort(id, upOrDown) {
        	...
        	$.ajax({
	    		   ...
	    		   url: "${ctx!}/admin/feedback/updateOrder",
